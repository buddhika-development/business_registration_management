  ...
  <div className="rounded-2xl border border-slate-200 bg-white p-5 animate-pulse">
    <div className="flex justify-between gap-3">
      <div className="flex-1">
        ...
        <div className="mt-2"><SkeletonBar w="w-48" /></div>
      </div>
...
export default function PendingBusinesses() {
  ...
  const [sendingKey, setSendingKey] = useState(null); // for Request Fix button
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setErr(null);
      try {
        const res = await fetch(`${base}/api/admin/business/pending`, {
          cache: "no-store",
  ...
  const requestFix = async (applicationno) => {
    setSendingKey(applicationno);
    setErr(null);
    try {
      const res = await fetch(`${base}/api/admin/business/${encodeURIComponent(applicationno)}/request-fix`, {
        method: "POST",
      });
      if (!res.ok) {
        const t = await res.text().catch(() => "");
  ...
  return (
    ...
    <div className="mt-8">
        <div className="mb-5">
            <MainTitle title={"Pending Approvals"} />
        </div>
        ...
        {!loading && !err && rows.map((b) => (
            <PendingBusinessCard
            key={b.applicationno}
            business={b}
            open={openKey === b.applicationno}
            onToggle={() => setOpenKey((k) => (k === b.applicationno ? null : b.applicationno))}
            onRequestFix={() => requestFix(b.applicationno)}
            requesting={sendingKey === b.applicationno}
            />
        ))}
        </div>
    </div>
